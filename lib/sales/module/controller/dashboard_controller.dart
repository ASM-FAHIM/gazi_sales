...
*/ /*  RxBool givingAtt = false.obs;
  ...
  Future<Object> getDealerInfo(String zid) async {
    try {
      isLoading(true);
      var response = await http.get(Uri.parse(
          'http://${AppConstants.baseurl}/gazi/salesforce/dealerinfo.php?xstaff=${loginController.xstaff.value}&zid=$zid'));
  ...
  void runFilter(String keyword) async {
    try {
      ...
      if (keyword.isEmpty) {
        ...
        result = list
            .where((dealer) => dealer["xorg"].toLowerCase().contains(keyword.toLowerCase()) ||
            dealer["xcus"].toLowerCase().contains(keyword.toLowerCase()) ||
            dealer["xphone"].toLowerCase().contains(keyword.toLowerCase()))
            .toList();
        isLoading5(false);
  ...
  Future<Object> insertIntoProductNature() async {
    try {
      isPNFetched(true);
      var response = await http.get(Uri.parse(
          'http://${AppConstants.baseurl}/gazi/salesforce/productNature.php?zid=${loginController.zID.value}&tso=${loginController.xstaff.value}'));
      if (response.statusCode == 200) {
        pNatureList = productNatureModelFromJson(response.body);
        //print("P nature: ${loginController.xstaff.value}");
        pNatureList.forEach((element) {
         //print(element.xcode);
          //print(element.xlong);
        });
  ...
  Future<Object> getProductInfo(String zid) async {
    try {
      isLoading3(true);
      //print('Zid for in API : ${zid}');
      var response = await http.get(Uri.parse(
          'http://${AppConstants.baseurl}/gazi/salesforce/productlist.php?zid=$zid'));
      if (response.statusCode == 200) {
  ...
  void runFilterForProduct(String keyword) async {
    try {
      ...
      if (keyword.isEmpty) {
        ...
        result = productList
            .where((dealer) => dealer["xdesc"].toLowerCase().contains(keyword.toLowerCase()) ||
            dealer["xitem"].toLowerCase().contains(keyword.toLowerCase()))
            // .where((name) =>
...
/*  var specProduct;
  ...
  Future<Object> getProductAccessoriesInfo(String zid) async {
    try {
      isPALoaded(true);
      var response = await http.get(Uri.parse(
          'http://${AppConstants.baseurl}/gazi/salesforce/ProductAccessoriesList.php?zid=$zid'));
      if (response.statusCode == 200) {
  ...
  /*Future<void> asyncFunction(String zid) async {
    isLoading7(true);
    DateTime startTime = DateTime.now();
    try {

      // Check if the zid exists in the dealer table
      int zidInt = int.parse(zid);
      bool zidExistsInDealerTable = await DatabaseRepo().checkZidExists(DBHelper.dealerTable, zidInt);
      if (!zidExistsInDealerTable) {
        // zid doesn't exist, delete all existing data from the dealer table
        await DatabaseRepo().deleteAllFromTable(DBHelper.dealerTable);
      }

      // Check if the zid exists in the product table
      bool zidExistsInProductTable = await DatabaseRepo().checkZidExists(DBHelper.productTable, zid as int);
      if (!zidExistsInProductTable) {
        // zid doesn't exist, delete all existing data from the product table
        await DatabaseRepo().deleteAllFromTable(DBHelper.productTable);
      }

      // Check if the zid exists in the product table
      bool zidExistsInProductAccessoriesTable = await DatabaseRepo().checkZidExists(DBHelper.productAccessories, zid as int);
      if (!zidExistsInProductAccessoriesTable) {
        // zid doesn't exist, delete all existing data from the product table
        await DatabaseRepo().deleteAllFromTable(DBHelper.productAccessories);
      }

      // Check if the zid exists in the product table
      bool zidExistsInProductNatureTable = await DatabaseRepo().checkZidExists(DBHelper.productNature, zid as int);
      if (!zidExistsInProductNatureTable) {
        // zid doesn't exist, delete all existing data from the product table
        await DatabaseRepo().deleteAllFromTable(DBHelper.productNature);
      }

      await getDealerInfo(zid);
      // await getProductInfo(zid);
      // await getProductAccessoriesInfo(zid);
  ...
  Future<void> asyncFunction(String zid) async {
    ...
    DateTime startTime = DateTime.now();
    try {
      // int zidInt = int.parse(zid); // Parse zid from String to int
      //
      // // Check if the zid exists in the dealer table
      // bool zidExistsInDealerTable = await DatabaseRepo().checkZidExists(DBHelper.dealerTable, zidInt);
      // if (!zidExistsInDealerTable) {
      ...
      //
      // // Check if the zid exists in the product table
      // bool zidExistsInProductTable = await DatabaseRepo().checkZidExists(DBHelper.productTable, zidInt);
      // if (!zidExistsInProductTable) {
      //   // zid doesn't exist, delete all existing data from the product table
      //   await DatabaseRepo().deleteAllFromTable(DBHelper.productTable);
      // }
      //
      // // Check if the zid exists in the product table
      // bool zidExistsInProductAccessoriesTable = await DatabaseRepo().checkZidExists(DBHelper.productAccessories, zidInt);
      // if (!zidExistsInProductAccessoriesTable) {
      //   // zid doesn't exist, delete all existing data from the product table
      //   await DatabaseRepo().deleteAllFromTable(DBHelper.productAccessories);
      // }
      //
      // // Check if the zid exists in the product table
      // bool zidExistsInProductNatureTable = await DatabaseRepo().checkZidExists(DBHelper.productNature, zidInt);
      // if (!zidExistsInProductNatureTable) {
      //   // zid doesn't exist, delete all existing data from the product table
      //   await DatabaseRepo().deleteAllFromTable(DBHelper.productNature);
      // }
      ...
      await DatabaseRepo().deleteAllFromTable(DBHelper.productAccessories);
      await DatabaseRepo().deleteAllFromTable(DBHelper.productNature);
      await DatabaseRepo().deleteAllFromTable(DBHelper.cartTable);
      await DatabaseRepo().deleteAllFromTable(DBHelper.cartDetailsTable);
      await DatabaseRepo().deleteAllFromTable(DBHelper.cartAccessoriesTable);
      await getDealerInfo(zid);
      await getProductInfo(zid);
      await getProductAccessoriesInfo(zid);
      await insertIntoProductNature();
